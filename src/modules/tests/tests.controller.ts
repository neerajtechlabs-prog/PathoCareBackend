import { Body, Controller, Delete, Get, Headers, Param, Post, Put, Query, Req, UseGuards, UploadedFile, UseInterceptors } from '@nestjs/common';
...
import { CreateTestDto, UpdateTestDto, CreateTestParameterDto, UpdateTestParameterDto } from './dto';
...
export class TestsController {
  ...
  @ApiBearerAuth()
  async create(@Headers('x-tenant-slug') tenantSlug: string, @Body() body: CreateTestDto, @Req() req: Request & { user?: { sub?: string } }) {
  ...
  @Roles(UserRole.LAB_ADMIN, UserRole.SUPER_ADMIN)
  @ApiBearerAuth()
  async update(@Headers('x-tenant-slug') tenantSlug: string, @Param('id') id: string, @Body() body: UpdateTestDto, @Req() req: Request & { user?: { sub?: string } }) {
  ...
  @ApiBearerAuth()
  async delete(@Headers('x-tenant-slug') tenantSlug: string, @Param('id') id: string, @Req() req: Request & { user?: { sub?: string } }) {
  ...
  @Roles(UserRole.LAB_ADMIN, UserRole.SUPER_ADMIN)
  @ApiBearerAuth()
  async createParameter(@Headers('x-tenant-slug') tenantSlug: string, @Param('id') testId: string, @Body() body: CreateTestParameterDto, @Req() req: Request & { user?: { sub?: string } }) {
  ...
  @Roles(UserRole.LAB_ADMIN, UserRole.SUPER_ADMIN)
  @ApiBearerAuth()
  async updateParameter(@Headers('x-tenant-slug') tenantSlug: string, @Param('parameterId') parameterId: string, @Body() body: UpdateTestParameterDto, @Req() req: Request & { user?: { sub?: string } }) {
    return this.testsService.updateParameter(tenantSlug, parameterId, body, req.user?.sub || 'system');
  }
  ...
  @Roles(UserRole.SUPER_ADMIN)
  @ApiBearerAuth()
  async deleteParameter(@Headers('x-tenant-slug') tenantSlug: string, @Param('parameterId') parameterId: string, @Req() req: Request & { user?: { sub?: string } }) {
